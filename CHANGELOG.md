...
- Уровень детализации лога запуска `agent.log.level` — по аналогии с уровнями логирования,
  ...
  - `compact` (по умолчанию) — из событий в `<buildDir>/agentic-cli-runner/<runId>.json`
    рекурсивно вырезаются служебные поля `uuid`, `session_id` и `usage` (идентификаторы
    запуска/сессии и статистика по токенам), которые не нужны при чтении лога;
  - `full` — полный лог: все события `stream-json` как есть (прежнее поведение).
- Новый enum `config.AgentLogLevel` и метод `AgentRunnerProperties.getLogLevel(...)`.
