...
### Исправлено
- `agent.log.level=compact` теперь применяется и к `AgentResultDto.getEventsJson()` — строке,
  которую обычно передают дальше (например, в контекст модели-судьи). Раньше уровень влиял
  только на файл лога и строку `[AGENT_RESPONSE]`, а `getEventsJson()` всегда возвращал полный
  необрезанный JSON — то есть compact не экономил токены там, где это важнее всего.

### Изменено
- Расширен список служебных полей, вырезаемых в `compact`: к `uuid`, `session_id`, `usage`
  добавлены `id`, `model`, `tool_use_id`, `parent_tool_use_id` — идентификаторы, ничего не
  дающие при чтении лога или модели-судье.
- `AgentResultDto.getEvents()` (список `JsonNode`) по-прежнему отдаёт полные исходные события;
  уровень влияет только на их сериализацию `getEventsJson()`.

### Добавлено
- Имя модели выносится один раз в поле `model` в шапке compact-лога (`AgentRunLogDto`) вместо
  повторения в каждом событии. Утиль `EventCompactor.extractModel(...)`.

## [1.4.0] — 2026-09-11

### Добавлено
- Уровень детализации лога запуска `agent.log.level` — по аналогии с уровнями логирования,
  но уровней всего два:
  - `compact` (по умолчанию) — из событий рекурсивно вырезаются служебные поля `uuid`,
    `session_id` и `usage` (идентификаторы запуска/сессии и статистика по токенам), которые
    не нужны при чтении лога;
  - `full` — полный лог: все события `stream-json` как есть (прежнее поведение).
- Уровень применяется к обоим выводам событий: файлу `<buildDir>/agentic-cli-runner/<runId>.json`
  и строке `[AGENT_RESPONSE]` в slf4j-логе. Программный результат `AgentResultDto`
  (`getEvents()` / `getEventsJson()`) всегда полный — уровень на него не влияет.
- Новый enum `config.AgentLogLevel`, метод `AgentRunnerProperties.getLogLevel(...)` и утиль
  `log.EventCompactor` (вырезание служебных полей событий).

### Изменено
- Уровень лога по умолчанию — `compact` (раньше лог всегда писался полностью). Для прежнего
  поведения задайте `agent.log.level=full`.
...
### Добавлено
- Таймаут выполнения агента настраивается через свойство `agent.timeout` (в минутах) в
  `agent-runner.properties`. Проперти переопределяет таймаут фабрики; если ключ не задан —
  используется значение по умолчанию. Читается методом `AgentRunnerProperties.getTimeout(...)`.

### Изменено
- Таймаут по умолчанию (`AgentRunnerImpl.DEFAULT_TIMEOUT`) увеличен с 3 до 15 минут.
